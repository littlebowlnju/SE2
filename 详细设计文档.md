...
|  项卓奇  | 2020.06.18 | 添加了业务逻辑层hotelbl模块 | V1.0.1 |
...
<table class="lake-table">
  <tbody>
    ...
    <tr>
      <td colspan="1" style="text-align: center; background-color: #FFFFFF;">
        <p>HotelMapper.getAllManagers()</p>
      </td>
      <td colspan="2" style="text-align: center; background-color: #FFFFFF;">
        <p>以列表形式获取所有身份类型为HotelManager的用户单一持久化对象</p>
      </td>
    ...
    <tr>
      <td colspan="1" style="text-align: center; background-color: #FFFFFF;">
        <p>HotelMapper.changeHotelManager(int hotelId,Integer managerId)</p>
      </td>
      <td colspan="2" rowspan="1" style="text-align: center; background-color: #FFFFFF;">
         指定managerId为hotelId对应酒店对象的管理员ID属性值
      </td>
...
<table border="3">
    ...
    <tr>
        <td rowspan="3"> OrderServiceImpl.add </td>
        <td> 语法</td>
    ...
    <tr>
        <td rowspan="3"> OrderServiceImpl.annul </td>
        <td> 语法</td>
    ...
    <tr>
        <td rowspan="3"> OrderServiceImpl.processOrder </td>
        <td> 语法</td>
...
|vo.OrderVO|订单类|
|OrderMapper.addOrder(OrderVO orderVO) |新增一个订单持久化对象|
|OrderMapper.getAllOrders()|以列表形式获取所有订单单一持久化对象|
|OrderMapper.annulOrder(int orderid)|根据订单号删除一个订单持久化对象|
|OrderMapper.processOrder(int orderid)|根据订单号处理一个订单持久化对象|
|OrderMapper.getOrderById(int orderid)|根据订单号获取一个订单持久化对象|

(4)业务逻辑层的动态模型

下图表明了NJUSE酒店管理系统中，当客户填写好需要添加的订单表单时，用户业务逻辑处理的相关对象之间的协作。
